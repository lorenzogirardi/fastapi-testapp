...
workflows in Markdown. Compiler `v0.86.2` in this repo.
...
the configured provider (here, OpenCode Zen). Holds provider secrets.
...
provider. Here, `COPILOT_PROVIDER_*` env vars route Copilot-engine calls to OpenCode Zen.

**Copilot engine (`engine: copilot`)** — The agent harness used by gh-aw in this repo.
Despite the name, the actual model is `hy3-free` served by OpenCode Zen, reached via
Copilot's BYOK provider hook.

**OpenCode Zen** — The external LLM provider endpoint (`https://opencode.ai/zen/v1`)
that actually generates the agent's responses, using model `hy3-free`.

**`hy3-free`** — The specific model identifier answered by OpenCode Zen for these
workflows (`model: copilot/hy3-free`).
...
**`OPENCODE_API_KEY`** — Repository secret holding the provider credential; injected as
...
`opencode.ai`). Limits where the agent can send requests.
...
is unknown; required here or the proxy rejects `hy3-free` (HTTP 400).
